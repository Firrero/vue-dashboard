...
<script>
	...
	methods: {
		removeRecord(){
			...
			api
				.request('post', '/record/removeRecord', { uid, token })
				.then(response => {
				this.getRecords()
		})
		.catch(error => {
		...
		getRecords() {
			...
			api
				.request('post', '/record/getRecord', { query, userId, token })
				.then(response => {
		...
		createRecord() {
			...
			api
				.request('post', '/record/create', { uid, date, project, activity, hours, feature, id, extra, userId, token })
				.then(response => {
				/* if success exists */
